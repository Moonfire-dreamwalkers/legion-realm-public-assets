/**
 * Legion Realm — Auth UI Module
 * 
 * Manages the Discord login button in the navigation bar.
 * 
 * Behavior:
 *   - Not logged in: Shows "Sign in with Discord" button → redirects to /api/auth/login
 *   - Logged in: Shows Discord username → clicking logs out via /api/auth/logout
 * 
 * Loads after siteConfig is available. Uses window.siteConfig.discordConfig.
 */
...
(function () {
  ...
  // ─── Render (uses DOM APIs to avoid XSS via innerHTML) ──────────────────
  ...
  function renderAuthButton() {
    ...
    if (authState.user) {
      ...
      nameSpan.textContent = authState.user.username;
      link.appendChild(nameSpan);

      link.title = "Click to log out";
      link.setAttribute("aria-label", "Logged in as " + authState.user.username + ". Click to log out.");
      link.classList.add("is-authenticated");
      link.classList.remove("is-unauthenticated");
    } else {
      // Logged out state
      var iconSpan = document.createElement("span");
      iconSpan.innerHTML = DISCORD_ICON_SVG;
      link.appendChild(iconSpan);

      var labelSpan = document.createElement("span");
      labelSpan.className = "auth-login-label";
      labelSpan.textContent = "Sign in";
      link.appendChild(labelSpan);

      link.title = "Sign in with Discord";
      link.setAttribute("aria-label", "Sign in with Discord");
      link.classList.add("is-unauthenticated");
  ...
  function checkSession() {
    fetch(AUTH_API + "/me", { credentials: "same-origin" })
      .then(function (res) {
        if (!res.ok) throw new Error("Session check returned " + res.status);
        return res.json();
      })
      .then(function (data) {
  ...
  // ─── Handle auth error in URL (from failed callback redirect) ───────────

  function checkAuthError() {
    var params = new URLSearchParams(location.search);
    var error = params.get("auth_error");
    if (error) {
      console.warn("[auth] OAuth error from callback:", error);
